...
def prepare_tasks(frame: pd.DataFrame, seed: int):
    ...
    return {"topic": topic_rows, "emotion": emotion_rows, "slots": slot_rows}
...
def evaluate_slots(model, loader, device, label_count):
    model.eval()
...
def train_slots(samples, args, device):
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            optimizer.step()
            total += loss.item()
        validation_loss, validation_f1 = evaluate_slots(model, validation_loader, device, len(labels))
        history.append({"epoch": epoch + 1, "train_loss": total / len(train_loader), "validation_loss": validation_loss, "validation_slot_macro_f1": validation_f1})
        if validation_loss < best_loss:
            best_loss = validation_loss
            best_state = {key: value.detach().cpu().clone() for key, value in model.state_dict().items()}
    model.load_state_dict(best_state)
    model.to(device)
    test_loss, test_f1 = evaluate_slots(model, test_loader, device, len(labels))
    output = args.output / "slots"
    output.mkdir(parents=True, exist_ok=True)
    model.save_pretrained(output)
    tokenizer.save_pretrained(output)
    metrics = {"task": "BIO semantic slot extraction", "algorithm": "fine-tuned transformer token classification", "labels": labels, "rows": len(samples), "test_loss": test_loss, "test_slot_macro_f1_excluding_o": test_f1, "history": history}
...
def train_task(task, rows, args, device):
    ...
    for epoch in range(args.epochs):
        ...
        validation_loss, validation_f1, _, _ = evaluate(model, validation_loader, device, multilabel)
        history.append({"epoch": epoch + 1, "train_loss": training_loss / len(train_loader), "validation_loss": validation_loss, "validation_micro_f1": validation_f1})
        if validation_loss < best_loss:
            best_loss = validation_loss
            best_state = {key: value.detach().cpu().clone() for key, value in model.state_dict().items()}
    model.load_state_dict(best_state)
    model.to(device)
    test_loss, test_f1, targets, predictions = evaluate(model, test_loader, device, multilabel)
